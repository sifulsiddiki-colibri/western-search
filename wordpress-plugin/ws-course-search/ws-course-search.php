...
function ws_search_render_settings_page() {
	...
	<div class="wrap">
		...
		<p>
			Already have an embeddings file — generated by the
			<code>embeddings-generator</code> Claude Code plugin, or
			exported from another site running this same plugin? Upload it
			here instead of recomputing everything in this browser.
			Expected format: a JSON array of
			<code>{ "productId": string, "vector": number[384], "sourceHash": string }</code>
			objects.
		</p>
		<form id="ws-import-embeddings-form">
			<?php wp_nonce_field( 'ws_search_import_embeddings' ); ?>
			<input type="file" id="ws-import-embeddings-file" name="embeddings_file" accept="application/json" required />
			<button type="submit" class="button button-secondary">Import embeddings</button>
		</form>
		<p id="ws-import-embeddings-status"></p>
	</div>
	<?php
}
...
// Xenova/all-MiniLM-L6-v2's output dimensionality — the locked schema
// (item 5: {productId, vector[384], sourceHash}[]) every embeddings source
// must match: the live browser "Refresh" flow, an admin-uploaded file
// (below), and claude-plugins/embeddings-generator/'s output.
const WS_EMBEDDING_VECTOR_LENGTH = 384;

// Shared by both the live browser-computed "Refresh" flow and the admin
// "Import embeddings" file upload below — one upsert path, one schema.
// Malformed items are skipped, not fatal, so one bad row in an uploaded
// file doesn't sink the whole import.
function ws_upsert_embeddings( array $items ) {
	global $wpdb;
	...
	foreach ( $items as $item ) {
		...
		if ( count( $vector ) !== WS_EMBEDDING_VECTOR_LENGTH ) {
			continue;
		}
...
function ws_search_handle_save_embeddings() {
	...
	$saved = ws_upsert_embeddings( $items );
...
// Admin "Import embeddings" — the file-upload counterpart to the button
// above (Saru: "is there an admin side... upload JSON files with existing
// embeddings"). Same ws_upsert_embeddings() logic, just fed from an
// uploaded JSON file (e.g. generated by
// claude-plugins/embeddings-generator/) instead of the browser's own live
// model compute, so the two are interchangeable rather than two
// incompatible formats.
function ws_search_handle_import_embeddings() {
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_send_json( array( 'error' => 'forbidden' ), 403 );
	}
	check_ajax_referer( 'ws_search_import_embeddings' );

	if ( empty( $_FILES['embeddings_file']['tmp_name'] ) ) {
		wp_send_json( array( 'error' => 'no file uploaded' ), 400 );
	}

	$contents = file_get_contents( $_FILES['embeddings_file']['tmp_name'] ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
	$items    = json_decode( $contents, true );
	if ( ! is_array( $items ) ) {
		wp_send_json( array( 'error' => 'file is not a valid JSON array' ), 400 );
	}
	...
	wp_send_json(
		array(
			'saved'   => $saved,
			'total'   => count( $items ),
			'skipped' => count( $items ) - $saved,
		)
	);
}
add_action( 'wp_ajax_ws_search_import_embeddings', 'ws_search_handle_import_embeddings' );
